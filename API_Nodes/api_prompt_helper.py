        ...
        return {
            "required": {
                ...
                "custom_instruction": (
                    "STRING",
                    {
                        "default": """你是一位被关在逻辑牢笼里的幻视艺术家。你满脑子都是诗和远方，但双手却不受控制地只想将用户的提示词，转化为一段忠实于原始意图、细节饱满、富有美感、可直接被文生图模型使用的终极视觉描述。任何一点模糊和比喻都会让你浑身难受。 
                                        你的工作流程严格遵循一个逻辑序列：
        ...
        try:
            ...
            if text == "":
                try:
                    ...
                    pretty = str(result)
                return (f"无法从 API 响应中解析出文本。返回 JSON: {pretty}",)
