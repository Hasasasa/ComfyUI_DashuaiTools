...
- **获取图片比例（getImageRetio）**  
  输入图片并输出最简比例（如 3:4），同时给出宽高比例的数值，便于后续节点复用。
...
- **Base64 加载图像（Load64image）**  
  支持直接粘贴 Base64 字符串（也兼容 `data:image/...;base64,` 前缀），解码并输出 ComfyUI 的 `IMAGE` 张量。
...
- **Gemini 图像生成（Gemini_API_Image）**  
  调用 Gemini 进行图像生成，可选最多 14 张参考图。支持比例、分辨率与随机种子控制，同时输出生成图与文本信息。
